...
function PAMA_scanProject() {
  try {
    ...
    for (var i = 1; i <= project.numItems; i++) {
      ...
      // Create a common data object for the items array
      var itemData = {
        index: i,
        name: item.name
      };
...
// Load the Bodymovin initializer to enable import functionality
// This loads all the JSX functions needed for the BodymovinImporter.convert to work
try {
  $.writeln("PAMA: Loading Bodymovin initializer for import functionality...");
  var initializerFile = new File($.fileName);
  initializerFile = initializerFile.parent;
  initializerFile.changePath("initializer.jsx");

  if (initializerFile.exists) {
    $.evalFile(initializerFile.fsName);
    $.writeln("PAMA: Bodymovin initializer loaded successfully");
  } else {
    $.writeln("PAMA ERROR: initializer.jsx not found at: " + initializerFile.fsName);
  }
} catch (error) {
  $.writeln("PAMA ERROR loading initializer: " + error.toString());
}
